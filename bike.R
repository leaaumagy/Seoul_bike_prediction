...
## Variable d'interet et variables catégorielles : chi-squared tests
chi_day <- chisq.test(x = bike$Day, y = bike$Rented.Bike.Count)
chi_hours <- chisq.test(x = bike$Hour, y = bike$Rented.Bike.Count)
chi_seasons <- chisq.test(x = bike$Seasons, y = bike$Rented.Bike.Count)
chi_holidays <- chisq.test(x = bike$Holiday, y = bike$Rented.Bike.Count)
chi_Date <- chisq.test(x = bike$Date, y = bike$Rented.Bike.Count)

# Tableau de synthèse des tests de chi deux
chi_test_table <- data.frame(
  Variable = c("Day", "Hour", "Seasons", "Holiday"),
  Test_Statistic = c(chi_day$statistic, chi_hours$statistic, chi_seasons$statistic, chi_holidays$statistic),
  Degrees_of_Freedom = c(chi_day$parameter, chi_hours$parameter, chi_seasons$parameter, chi_holidays$parameter),
  P_Value = c(chi_day$p.value, chi_hours$p.value, chi_seasons$p.value, chi_holidays$p.value),
  Result = ifelse(c(chi_day$p.value, chi_hours$p.value, chi_seasons$p.value, chi_holidays$p.value) < 0.05, "Significant", "Not significant")
)
chi_test_table
#Les variables "Day" et "Holiday"  ne sont pas significative


##Variable d'intérêt et variable continue : Test ANOVA
test_anova <- aov(Rented.Bike.Count ~ Temperature.C. + 
                ...
                data = bike)
summary(test_anova)
#Les variable "snowfall", "Dew.point.temperature.C." et "Visibility..10m." ne sont pas significative
...
#Dans notre modèle de regression nous pouvons donc exclure les variables sont significatives
#On peut également exclure les variables qui ont des corrélations comme "Seasons"


# Analyse économétrique : Modèle MCO

#J'effectue une régression linéaire avec comme variable dépendante le nombre de vélos loué par heures et comme variable indépendantes, la température,visibilité,la pluie, l'ensoleillement et la neige.
